function transferRange(){
    ...
    var Mode = 0
    var Down = 0
    var timeNow = "2010-04-06 00:00:00"
    ...
    //         var timeNow = "2010-04-"+transfer(time_day+6)+ " " + transfer(time_hour) + ":" + transfer(time_minute) + ":" + transfer(time_second)
        ...
        // "--- width is ---" + containerSvgTrack.attributes.width.value + "---")
            ...
            var slider = d3.select("#timerange")
                ...
                .on("mousemove", function(){
                    ...
                    if(Mode == 0 && Down == 1){
                        $('#time').val(timeNow)
                        judgeStore(timeNumber)
                    }
                })
            

            function judgeStore(number){
                for (var i = 0; i <data.length; i++){
                    ...
                    if (longitude < xend && longitude >= xstart && latitude < yend && latitude >= ystart && timestamp == timeNow){
                        totalValue += value
                        lonplat['x'] = longitude
                        lonplat['y'] = latitude
                ...
                averageValue = totalValue / areaDotList.length
